...
async function startCapture() {
  try {
    ...
    let lastVoiceAt = 0;
    socket = new WebSocket(`${location.protocol === "https:" ? "wss" : "ws"}://${location.host}/ws/capture`);
    ...
    socket.onopen = () => {
      ...
      processor.onaudioprocess = (event) => {
        ...
        const level = rootMeanSquare(raw);
        const threshold = Math.max(0.006, noiseFloor * 3);
        const speech = level >= threshold;
        // Always send the timeline. Only use the energy gate for utterance
        // boundaries; removing silence from the audio makes Whisper merge words.
        const samples = downsample(raw, audioContext.sampleRate);
        socket.send(samples.buffer);
        if (speech) {
          lastVoiceAt = performance.now();
        } else {
          // Adapt to the room without treating background hum as a voice.
          noiseFloor = noiseFloor * 0.98 + Math.min(level, 0.012) * 0.02;
          if (lastVoiceAt && performance.now() - lastVoiceAt > 650) {
            socket.send(JSON.stringify({ type: "flush" }));
            lastVoiceAt = 0;
          }
        }
        // ScriptProcessor must be connected to run; explicitly keep it silent.
        event.outputBuffer.getChannelData(0).fill(0);
      };
    };
    socket.onmessage = async (event) => {
      const message = JSON.parse(event.data);
      if (message.type === "ready" && !message.asr_ready) {
        $("hint").textContent = "Microphone capture is ready. Add a local faster-whisper model to transcribe speech.";
      }
      if (message.type === "ready") setTranscriptMode(message.asr_ready ? "Listening" : "No model", message.asr_ready ? "live" : "warn");
      if (message.type === "transcribing") setTranscriptMode(message.backend === "sarvam-api" ? "Sarvam is transcribing…" : "Transcribing…", "busy");
      if (message.type === "transcript") {
        if (message.final) { partial = ""; setTranscriptMode("Listening", "live"); await loadRecent(); }
        else partial = message.text;
        renderTranscript();
      }
      if (message.type === "live_caption") {
        finalized.push(message.text);
        partial = "";
        renderTranscript();
      }
      if (message.type === "error") { captureSaveFailed = true; setStatus(message.message, "warn"); setTranscriptMode("Needs attention", "warn"); }
      if (message.type === "stopped") { setStatus(captureSaveFailed ? "Nothing saved" : "Saved locally", captureSaveFailed ? "warn" : ""); setTranscriptMode(captureSaveFailed ? "Not saved" : "Saved", captureSaveFailed ? "warn" : ""); }
    };
    socket.onclose = () => { if (isCapturing) finishCapture(); };
    finalized = []; partial = ""; captureSaveFailed = false; renderTranscript(); startTimer(); setCaptureButton(true); setStatus("Listening", "live");
  } catch (error) {
    stopAudio(); setStatus("Microphone unavailable", "warn"); $("hint").textContent = "Microphone permission is required to capture.";
  }
}
...
function finishCapture() {
  ...
  if (wasCapturing && socket && socket.readyState === WebSocket.OPEN) socket.send(JSON.stringify({ type: "stop" }));
}
...
async function loadSummary() {
  ...
  try {
    ...
    $("summary-text").textContent = "The summary could not be refreshed right now.";
  } finally { button.disabled = false; button.textContent = "Refresh summary ↗"; }
}
...
async function loadRecent() {
  ...
  $("memory-count").textContent = data.total ? `${data.total} saved` : "";
  list.replaceChildren();
  if (!data.memories.length) { list.textContent = "Your saved memories will appear here."; return; }
  data.memories.slice(0, 9).forEach((memory) => {
...
async function saveSarvamKey(event) {
  ...
  try {
    const response = await fetch("/api/sarvam", { method: "POST", headers: { "Content-Type": "application/json" }, body: JSON.stringify({ api_key: key, language_code: $("sarvam-language").value }) });
    const data = await response.json();
    if (!response.ok) throw new Error(data.detail || "The Sarvam API key could not be saved.");
    input.value = ""; activeAsr = data.active_asr || "asr_sarvam_ai"; setSarvamState(true, true); $("sarvam-key-status").textContent = `API saved securely · Currently using Sarvam AI · ${$("sarvam-language").selectedOptions[0].textContent}. The key is masked and never shown again.`;
    $("hint").textContent = "Sarvam AI is ready for speech-to-text."; setStatus("Sarvam ready"); await Promise.all([loadStatus(), loadModels()]);
  } catch (error) { $("sarvam-key-status").textContent = error.message || "The Sarvam API key could not be saved."; setStatus("Sarvam unavailable", "warn"); }
  button.disabled = false; button.textContent = "Save & use Sarvam";
}
...
$("chat-form").addEventListener("submit", async (event) => {
  ...
  $("answer").textContent = "Searching your memories…"; $("sources").replaceChildren();
  try {
    ...
    data.sources.forEach((source) => { const chip = document.createElement("span"); chip.className = "source"; chip.textContent = formatTime(source.created_at); $("sources").append(chip); });
  } catch (_) { $("answer").textContent = "I could not search your memories just now."; }
});
$("save-note").addEventListener("click", async () => {
  ...
  try {
    const response = await fetch("/api/memories", { method: "POST", headers: { "Content-Type": "application/json" }, body: JSON.stringify({ text }) });
    if (!response.ok) throw new Error(); input.value = ""; $("save-note").textContent = "Saved locally"; await loadRecent();
  } catch (_) { $("save-note").textContent = "Could not save"; }
...
Promise.all([loadStatus(), loadRecent(), loadModels()]);
